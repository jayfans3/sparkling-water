...
class H2OContext private (@transient val sparkContext: SparkContext, @transient conf: H2OConf) extends org.apache.spark.Logging
  ...
  def asDataFrame[T <: Frame](fr: T)(implicit sqlContext: SQLContext) : DataFrame = toDataFrame(this, fr)
  def asDataFrame(s : String)(implicit sqlContext: SQLContext) : DataFrame = toDataFrame(this, new H2OFrame(s))
